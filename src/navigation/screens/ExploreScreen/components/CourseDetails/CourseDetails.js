...
export default function CourseDetails({name, category, instructor, totalLessons, duration, price, rating, favourite}) {
  return (
    <View style={styles.container}>
      <Image source={courseDetailsThumbPlaceholder} style={styles.image} />
      <View style={styles.detailsWrapper}>
        <View style={styles.categoryFavourite}>
          <Text style={styles.category}>{category}</Text>
          <AntDesign
            name={favourite ? 'heart' : 'hearto'}
            color={favourite ? 'crimson' : 'darkgrey'}
            style={styles.favourite}
          />
        </View>
        
        <Text numberOfLines={1} style={styles.name}>{name}</Text>
        <Text numberOfLines={1} style={styles.instructor}>{instructor}</Text>
